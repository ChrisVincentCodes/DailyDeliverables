$(document).ready(function(){
    ...
    // Declare oscilloscope node.
    oscilloscopeNode = new WavyJones(context, 'oscilloscope');
    oscilloscopeNode.lineColor = '#70DBDB';
    oscilloscopeNode.lineThickness = 3;
    oscilloscopeNode.connect(context.destination);

	 	// Declare gain node.
	  var gainNode = context.createGain();
	  gainNode.gain.value = 0.1;
	  gainNode.connect(oscilloscopeNode);

    

	  // Declare analyzer node.
	  // var analyzerNode = context.createAnalyzer();
  ...
  $('#freqControlTitle').click(function(){
    shrinkOption('#frequencyControls');
  })
  $('#detuneControlTitle').click(function(){
    shrinkOption('#detuneControls');
  })
  $('#waveControlTitle').click(function(){
    shrinkOption('#waveControls');
  })
  $('#gainControlTitle').click(function(){
    shrinkOption('#gainControls');
  })

  function shrinkOption(selector) {
    /*window.alert("yo");*/
    $(selector).toggleClass('shrunk', 800, 'easeInOutQuad');
    /*$(selector +".controlTitle").css('opacity', 1;});*/
